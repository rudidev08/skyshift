...
function buildToolbarHtml(economyEditingIsEnabled: boolean): string {
  ...
  html += '<select id="draft-select" class="editor-compact-select"><option value="">— drafts —</option></select>';
  html += '<button id="load-draft-button">Load Draft</button>';
  html += '<button id="delete-draft-button" class="button-danger" title="Delete selected draft">Delete</button>';
  html += '</div>';
  if (!economyEditingIsEnabled) {
    html += '<div class="toolbar-divider"></div>';
    html += '<div class="toolbar-group toolbar-group-note">';
    html += '<span class="toolbar-badge">Read-only</span>';
    html += '<span class="toolbar-note">Editing, save, revert, and drafts are available in local dev only.</span>';
    html += '</div>';
  }
  html += '</div>';
  return html;
...
function buildSimulationPanelHtml(): string {
  ...
  html += '<span class="sim-label">Hours:</span> <input type="number" id="simulation-hours" value="20" step="1" min="1" class="editor-compact-number-input">';
...
export function buildEditorPageHtml(
  editableStations: StationPlacement[],
  economyEditingIsEnabled: boolean,
): string {
  let html = buildToolbarHtml(economyEditingIsEnabled);

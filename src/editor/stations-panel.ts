...
import type { StationPlacement, StationTypeId, StationSize } from "../../data/station-types";
...
function buildAddStationControlsHtml(allPlayableNations: Nation[]): string {
  ...
  html += "</select>";
  html += '<select id="add-size"><option value="S">S</option><option value="M">M</option><option value="L">L</option></select>';
  html += '<button id="add-station-button">+ Add Station</button>';
  html += '</div>';
  html += '<div class="panel-note">Added stations are temporary simulation-only and are not saved to files or drafts.</div>';
  html += '</div>';
  return html;
}
...
function sortStationIndicesByNation(editableStations: StationPlacement[], allPlayableNations: Nation[]): number[] {
  ...
  return editableStations
    ...
    .sort((leftIndex, rightIndex) => {
      ...
      const nationDifference = (nationOrder.get(leftStation.nation.id) ?? 999) - (nationOrder.get(rightStation.nation.id) ?? 999);
...
function buildStationRow(stationRowInput: {
  ...
  let html = "<tr>";
  html += `<td><button class="remove-button" data-action="remove-station" data-station-index="${stationIndex}" title="Remove station">×</button></td>`;
  const nameClass = stationHealthClass ? ` class="${stationHealthClass}"` : "";
  html += `<td${nameClass} style="color:${stationHealthClass ? "" : mapStation.nation.color}">${mapStation.nation.codeName} ${mapStation.name ?? mapStation.id} <span class="size-badge">${mapStation.size}</span></td>`;
  html += `<td>${stationData.stationType.name}</td>`;
  ...
  for (const ware of wareColumns) {
    ...
    if (net !== 0) {
      ...
      html += `<td class="ware-cell ${rateClass}">${sign}${net.toFixed(1)}</td>`;
    } else {
      html += '<td class="ware-cell"></td>';
    }
  }
  html += "</tr>";

  return { html, produced, consumed };
}

function mergeWareTotals(accumulator: Map<WareId, number>, fromRow: Map<WareId, number>): void {
  for (const [wareId, amount] of fromRow) {
    accumulator.set(wareId, (accumulator.get(wareId) ?? 0) + amount);
  }
}

function buildStationTotalsRowsHtml(wareColumns: typeof allWares, totalProduced: Map<WareId, number>, totalConsumed: Map<WareId, number>): string {
  let html = "";
...
function buildStationTableBodyHtml(
  ...
  for (const stationIndex of sortedStationIndices) {
    ...
    const row = buildStationRow({ mapStation, stationData, rates, wareColumns, stationHealthClass, stationIndex });
    html += row.html;
    mergeWareTotals(totalProduced, row.produced);
    mergeWareTotals(totalConsumed, row.consumed);
  }

  return { html, totalProduced, totalConsumed };
}
...
function buildStationPanelHtml(
  ...
  const body = buildStationTableBodyHtml(mapState, simulationSession, allPlayableNations, wareColumns, totalColumnCount);
  html += body.html;
...
export function addStation(dependencies: AddStationDependencies) {
  ...
  stationCounter++;
  const stationType = stationTypes.find(candidate => candidate.id === stationTypeId);
  const newStation: StationPlacement = {
    id: `custom-${stationCounter}`,
    name: `${stationType?.name ?? stationTypeId} ${stationCounter}`,
    x: 0,
    y: 0,
    nation,
    stationTypeId,

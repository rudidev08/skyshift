...
import { stationTypes } from "../../../data/stations.ts";
...
type StationType = typeof stationTypes[number];
type Station = ReturnType<typeof createStation>;

type ProductionResult =
  | { kind: "sink"; ware: Ware; stationType: StationType }
  | { kind: "raw"; ware: Ware; effectiveOutput: number; outputMax: number; fillCycles: number; timeSeconds: number }
  | {
      kind: "simulated";
      ware: Ware;
...
function buildPrimedSimStation(ware: Ware, stationType: StationType): Station {
  const simStationPlacement = {
    id: `sim-${ware.id}`, name: "Sim", x: 0, y: 0,
    nation: hubNation, stationTypeId: stationType.id, size: "S" as const,
  };
  const station = createStation(simStationPlacement);
...
function runProductionForWare(ware: Ware): ProductionResult | null {
  const stationType = stationTypes.find((candidate) => candidate.produces.includes(ware.id));
  if (!stationType) return null;

  const station = buildPrimedSimStation(ware, stationType);
  ...
  if (ware.productionInputs.length === 0) {
    ...
    return { kind: "raw", ware, effectiveOutput, outputMax: outputSlot.max, fillCycles, timeSeconds };
  }

  const reportTimer = new EconomyTimer();
  let cycles = 0;
  const maxCycles = Math.ceil(economyConfig.targetFillTimeSeconds / economyConfig.simulationIntervalSeconds) * 2;
...
function printProductionResult(result: ProductionResult): void {
  if (result.kind === "sink") {
    ...
    console.log(`    No output storage slot. ${result.stationType.name} only consumes inputs in live stations.\n`);
    return;
  }

  if (result.kind === "raw") {
    console.log(`  ${result.ware.name} — per station (output ${result.effectiveOutput}/cycle)`);
    console.log(`    Output storage: ${result.outputMax}  →  fills in ${result.fillCycles} cycles (${result.timeSeconds}s)`);
    console.log(`    ✓ Raw resource — always fills\n`);
    return;
  }

  console.log(`  ${result.ware.name} — per station (output ${result.effectiveOutput}/cycle)`);
  console.log(`    Output storage: ${result.outputMax}  →  ${result.outputCurrent}/${result.outputMax} (${result.outputPercent}%) in ${result.cycles} cycles (${result.timeSeconds}s)`);
  if (result.filled) {
    console.log(`    ✓ Fills completely`);
  } else {
    const emptyInput = getAllInventorySlots(result.station).find((slot) =>
      !result.stationType.produces.includes(slot.ware.id) && slot.current <= 0,
    );
    console.log(`    ✗ Stalls at ${result.outputPercent}% — ${emptyInput?.ware.name ?? "unknown"} runs out`);
  }
  ...
  for (const slot of getAllInventorySlots(result.station)) {
    ...
    console.log(`    ${slot.ware.name.padEnd(12)} storage: ${slot.max}  →  ${slot.current}/${slot.max} remaining (${remainingPercent}%)`);
  }
  console.log();
}
...
export function runProductionSimulation(): void {
  ...
  console.log(`  Target fill time: ${economyConfig.targetFillTimeSeconds}s (${economyConfig.targetFillTimeSeconds / 60} min)\n`);

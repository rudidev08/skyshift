...
export type ShipAction =
   ...
   *  save/load. `route` drives the info-card cargo-note only — trade flights
   *  set it so the card shows "Route: X to Y"; ferries, deploy legs, and other
   *  non-trade flights leave it undefined and the card falls back to `label`
   *  as a status line. */
  | {
      type: "fly";

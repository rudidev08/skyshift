...
export function computeBezierControlPoint(
  from: RatioPoint, to: RatioPoint,
): { controlX: number; controlY: number } {
  const side = Math.random() < 0.5 ? 1 : -1;
  const sideBias = side > 0
    ? 0.6  + Math.random() * 0.4
    : 0.15 + Math.random() * 0.45;
  const angleDegrees = CURVE_ANGLE_MIN_DEGREES + sideBias * (CURVE_ANGLE_MAX_DEGREES - CURVE_ANGLE_MIN_DEGREES);
  ...
  const controlX = Math.max(0.05, Math.min(0.95, (from.xRatio + to.xRatio) / 2 + perpendicularX * offsetDistance * side));
  const controlY = Math.max(0.05, Math.min(0.95, (from.yRatio + to.yRatio) / 2 + perpendicularY * offsetDistance * side));
  return { controlX, controlY };
}

/** Bezier-sample the flight at `progress`, returning normalized xRatio/yRatio. */
export function sampleFlightPosition(flight: FlightAnimation, progress: number): { xRatio: number; yRatio: number } {
  return {

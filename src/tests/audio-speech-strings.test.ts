...
test("text-to-speech override helpers convert roman numerals and digits into spoken words", () => {
  ...
  assertTrue(
    nameToVoiceKey("Medical Lab") === "medical-lab",
    "display names should become hyphenated voice keys",
  );
  assertTrue(
    applyTextToSpeechOverride("Hub-Cluster") === "Hub-Cluster",
    "non-overridden text should pass through unchanged",
  );
  assertTrue(
    nameToVoiceKey("Gap of Calamity") === "gap-of-calamity",
    "every whitespace run should become a hyphen, not just the first",
  );
  // Pin the `+` in `\s+`. Without it, consecutive whitespace chars each become
  // their own hyphen — "Hub  Cluster" would collapse to "hub--cluster" rather
  // than the expected single-hyphen "hub-cluster".
  assertTrue(
    nameToVoiceKey("Hub  Cluster") === "hub-cluster",
    "consecutive whitespace should collapse to a single hyphen",
  );
});
...
test("core announcement helpers derive keys and speech strings from canonical game data", () => {
  ...
  // Pin the per-nation stationNames pool. Skipping that loop would silently drop data-file station-name voice keys.
  assertTrue(voiceKeys.has("high-pinnacle"), "nation station name pool should become voice keys");
  // Pin the per-nation shipNames pool. Skipping that loop would silently drop data-file ship-name voice keys.
  ...
  // Pin the per-nation stationNames pool for speech strings. Skipping that loop would drop data-file station names.

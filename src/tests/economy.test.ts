...
function createTestStation(produces: WareId[], inventory: InventorySlot[], sizeMultiplier = 1): Station {
  ...
  return makeStation({ produces, inventory, sizeMultiplier });
}
...
test("production consumes inputs and produces output", () => {
  ...
  const iceSlot = createSlot(ice, 100, 500);
  const waterSlot = createSlot(water, 0, 500);
...
test("production stops when output storage is full", () => {
  const iceSlot = createSlot(ice, 100, 500);
  const waterSlot = createSlot(water, 500, 500);
  const station = createTestStation(["water"], [waterSlot, iceSlot]);
  ...
  // didProduceLastTick must reflect the skipped tick — UI/HUD reads this flag
  // to show idle vs busy state, so it has to flip to false when nothing fired.
...
test("production stops when inputs are insufficient", () => {
  ...
  const iceSlot = createSlot(ice, 3, 500);
  const waterSlot = createSlot(water, 0, 500);
  const station = createTestStation(["water"], [waterSlot, iceSlot]);
  ...
  assertEqual(station.didProduceLastTick, false, "didProduceLastTick should be false when inputs are insufficient");
});
...
test("production fires when input quantity exactly matches cost", () => {
  ...
  const iceSlot = createSlot(ice, 8, 500);
  const waterSlot = createSlot(water, 0, 500);
  const station = createTestStation(["water"], [waterSlot, iceSlot]);
...
test("output is clamped to max storage", () => {
  ...
  const iceSlot = createSlot(ice, 100, 500);
  const waterSlot = createSlot(water, 498, 500);
  const station = createTestStation(["water"], [waterSlot, iceSlot]);
...
test("provisions producer consumes food/medicine and emits provisions each tick", () => {
  ...
  const foodSlot = createSlot(food, 50, 500);
  const medicineSlot = createSlot(medicine, 50, 500);
  const provisionsSlot = createSlot(provisions, 0, 500);
...
test("staggerStationTicks spreads first ticks across one interval", () => {
  ...
  assertEqual(stations[1].secondsSinceLastTick, -(interval * 1 / 3), "second station offset by -1/3 interval");
  assertEqual(stations[2].secondsSinceLastTick, -(interval * 2 / 3), "third station offset by -2/3 interval");
});
...
test("EconomyTimer.reset zeroes both tick counter and sub-tick accumulator", () => {
  ...
  assertTrue(timer.tick > 0, "precondition: timer advanced before reset");

  timer.reset();

  assertEqual(timer.tick, 0, "tick reset to 0");
  ...
  tickEconomy([], timer, economyConfig.simulationIntervalSeconds - 0.1);
  assertEqual(timer.tick, 0, "sub-tick accumulator reset to 0 (no early tick)");
});
...
test("EconomyTimer advances one tick per accumulated interval", () => {
  ...
  const timer = new EconomyTimer();
  assertEqual(timer.tick, 0, "tick counter starts at 0");

  tickEconomy([], timer, 0.25);
  assertEqual(timer.tick, 0, "no tick yet after 0.25s");

  tickEconomy([], timer, 0.25);
  assertEqual(timer.tick, 1, "tick fires at 0.5s accumulated");

  tickEconomy([], timer, 0.25);
  assertEqual(timer.tick, 1, "no second tick after partial advance");

  tickEconomy([], timer, 0.25);
  assertEqual(timer.tick, 2, "second tick at 1.0s accumulated");
});

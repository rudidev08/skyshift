...
test("hasUnsavedEdits: fresh editor with untouched baseline returns false", () => {
  ...
  assertTrue(
    !hasUnsavedEdits({
      stations: { current: currentStations, baseline: baselineStations },
      nebulas: { current: currentNebulas, baseline: baselineNebulas },
    }),
...
test("hasUnsavedEdits: station removed (current shorter than baseline) -> true", () => {
  ...
  assertTrue(
    hasUnsavedEdits({
      stations: { current, baseline },
      nebulas: { current: [], baseline: [] },
    }),
    "removing a station is an edit",
  );
});
...
test("hasUnsavedEdits: station id substituted (same count) -> true", () => {
  ...
  assertTrue(
    ...
    "substituting a new id should register",
  );
});
...
test("hasUnsavedEdits: station moved -> true", () => {
  ...
  assertTrue(
    ...
    "x-coordinate change should register",
  );
});
...
test("hasUnsavedEdits: station y moved (x unchanged) -> true", () => {
  ...
  assertTrue(
    ...
    "y-coordinate change should register",
  );
});
...
test("hasUnsavedEdits: nebula added -> true", () => {
  ...
  assertTrue(
    hasUnsavedEdits({
      stations: { current: [], baseline: baselineStations },
      nebulas: { current: currentNebulas, baseline: baselineNebulas },
    }),
...
test("hasUnsavedEdits: nebula moved -> true", () => {
  ...
  assertTrue(
    ...
    "nebula x change should register",
  );
});
...
test("hasUnsavedEdits: nebula y moved (x unchanged) -> true", () => {
  ...
  assertTrue(
    ...
    "nebula y change should register",
  );
});
...
test("hasUnsavedEdits: nebula texture swapped -> true", () => {
  ...
  assertTrue(
    ...
    "nebula texture change should register",
  );
});
...
test("hasUnsavedEdits: empty/empty returns false", () => {
  assertTrue(
    ...
    "nothing vs nothing is not an edit",
  );
});
...
test("hasUnsavedEdits: nebula at index >= 1 changed (index 0 unchanged) -> true", () => {
  ...
  assertTrue(
    ...
    "change at later index should register",
  );
});

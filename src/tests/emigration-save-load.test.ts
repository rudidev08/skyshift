...
test("snapshot mid-event: activeEvent fields roundtrip through fromSnapshot unchanged", () => {
  ...
  const event = source.simulation.emigrationManager.triggerEvent({ intensity: "high" });
  assertTrue(event !== null, "event triggered");

  const snapshot = captureSnapshot(source as never);

  const restored = setupFreshTestGame();
  restoreSavedGame(restored as never, snapshot);

  const restoredEvent = assertNotNull(
    restored.simulation.emigrationManager.getActiveEvent(),
    "restored event",
  );
  ...
  assertEqual(restoredEvent.startAt, event!.startAt, "startAt preserved");
...
test("snapshot mid-event after some ferries arrived: shipsArrived counter survives load", () => {
  ...
  for (let arrivalIndex = 0; arrivalIndex < 3; arrivalIndex++) {
    ...
    for (const observer of source.simulation.tradeManager.decommissionObservers) observer(synthetic);
  }
...
test("after load: decommission observer reattaches and continues counting arrivals", () => {
  ...
  for (const observer of restored.simulation.tradeManager.decommissionObservers) observer(synthetic);
...
test("snapshot post-demolition before WAY jump: load completes the jump on the next tick", () => {
  ...
  const restored = setupFreshTestGame();
  restoreSavedGame(restored as never, snapshot);
  restored.simulation.seedRosterForSavedGame(restored.stations, restored.ships);
  // Verify load preserved the jumpable state.
  assertNotNull(restored.simulation.emigrationManager.getActiveEvent(), "load preserves activeEvent");
  // Now tick once on the restored sim — the jump-check should fire.
  restored.simulation.slowSimulationTick(1);
  assertEqual(restored.simulation.emigrationManager.getActiveEvent(), null, "post-load tick fires the jump");
  assertTrue(
    restored.simulation.emigrationManager.getNextGenerationalShipArrivalAt() !== null,
...
test("nextEventCounter / nextEmigrantShipCounter / nextGenerationalShipCounter survive snapshot round-trip", () => {
  ...
  const restoredManagerSnapshot = restored.simulation.emigrationManager.toSnapshot();
  assertEqual(
...
test("activeGenerationalShipId survives snapshot round-trip and reattaches to the live ship", () => {
  ...
  const restoredShip = assertNotNull(
    restored.simulation.emigrationManager.getActiveGenerationalShip(),
    "restored gen ship",
  );
...
test("clockSeconds, mode, intensity, nextGenerationalShipArrivalAt all roundtrip", () => {
  ...
  const restoredState = restored.simulation.emigrationManager.toSnapshot();
  ...
  assertEqual(
    restoredState.nextGenerationalShipArrivalAt,
    sourceArrival,
    "nextGenerationalShipArrivalAt preserved",
  );
});

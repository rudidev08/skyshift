...
import { gameSecondsThisFrame } from "../game-loop.ts";
...
const orbit: OrbitState = { orbitAngleAtZero: 0.3, orbitSpeedRadPerSec: 1.2, orbitRadius: 50 };
...
test("game clock does not advance while paused — orbiting ships stay frozen", () => {
  ...
  gameClockSeconds += gameSecondsThisFrame(FRAME_DELTA_SECONDS, 0, false);
...
test("game clock advances at the real frame rate while running at 1x", () => {
  const advance = gameSecondsThisFrame(FRAME_DELTA_SECONDS, 1, false);
  assertEqual(advance, FRAME_DELTA_SECONDS, "1x advances by the raw frame delta");

  const poseStart = getOrbitingShipPose(ship, orbit, 0);
  const poseAfterFrame = getOrbitingShipPose(ship, orbit, advance);
...
test("game clock scales with game speed — 5x advances five times as far as 1x", () => {
  const advanceAt1x = gameSecondsThisFrame(FRAME_DELTA_SECONDS, 1, false);
  const advanceAt5x = gameSecondsThisFrame(FRAME_DELTA_SECONDS, 5, false);
  assertEqual(advanceAt5x, advanceAt1x * 5, "5x covers 5x the game seconds of 1x for the same real frame");
});

test("game clock stays frozen in the static editor even at 1x", () => {
  assertEqual(
    gameSecondsThisFrame(FRAME_DELTA_SECONDS, 1, true),
    0,
...
test("a negative speed is treated as paused, not as reverse time", () => {
  assertEqual(gameSecondsThisFrame(FRAME_DELTA_SECONDS, -1, false), 0, "negative speed does not rewind the clock");
});

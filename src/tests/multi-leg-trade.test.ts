...
// ware that the home consumes — current authored data has a strictly
...
function freshSettledSimulation(): Simulation {
...
function buildSyntheticTwoLegTrip(simulation: Simulation): { ship: TradeShip; legs: TradeTripLeg[] } | null {
...
test("startTrip with 2 legs places 4 reservations (outgoing + incoming for each)", () => {
  ...
  const candidate = buildSyntheticTwoLegTrip(simulation);
  assertTrue(candidate !== null, "synthetic 2-leg trip available");
  if (!candidate) { simulation.tradeManager.dispose(); return; }
...
test("startTrip: reservedOutgoing/reservedIncoming counters bump on stations at trip start", () => {
  ...
  const destinationSlot = assertNotUndefined(getInventorySlot(legs[0].toStation, legs[0].wareId), "dest slot");
...
test("trade-transfer observers fire with correct cargoDirection (outgoing at source, incoming at dest)", () => {
  ...
  for (let stepIndex = 0; stepIndex < 1500; stepIndex++) {
...
test("2-leg trip: per-ware reservations stay isolated to their station/direction (no cross-ware leakage)", () => {
  ...
  // Pin no cross-direction leak: home's medicine slot has NO incoming reservation
  // from this trip, target's food slot has no incoming, etc.
  // (These could be 0 or carry pre-existing reservations from the natural sim;
  // we assert the trip's amount didn't add to the wrong direction.)
  assertEqual(
    homeMedicineSlot.reservedIncoming - 0,
...
test("startTrip: tradeDirection is 'sell' when first leg starts at home, 'buy' otherwise", () => {
  ...
  assertEqual(ship.tradeDirection, expectedDirection, `firstLeg.fromStation=${legs[0].fromStation.id}, home=${ship.homeStationId} → tradeDirection=${expectedDirection}`);
  const expectedTarget = legs[0].fromStation.id === ship.homeStationId
    ? legs[0].toStation.id
    : legs[0].fromStation.id;
...
test("queue ends with an orbit-at-home local hop (ship lands in orbit, not on the surface)", () => {
  // Pin the trailing orbit hop. Mutating buildQueueFromTrip to drop it would
...
test("queue contains exactly 2 inter-station fly actions (home→target and target→home)", () => {
  // Pin the queue's between-stations flight count. Mutating buildQueueFromTrip
  // to drop the return flight would leave the ship stranded at the target.
  const simulation = freshSettledSimulation();
  const candidate = findShipWithNaturalTrip(simulation);
  assertTrue(candidate !== null, "natural trip");
  if (!candidate) { simulation.tradeManager.dispose(); return; }
  ...
  let interStationFlights = 0;
  for (const action of ship.actionQueue) {
    if (action.type === "fly" && action.travelMode === "interStation") interStationFlights++;
  }
  assertEqual(interStationFlights, 2, "2 inter-station flights (home→target and target→home)");
...
test("save mid-flight: ship.flight survives roundtrip and the queue tail remains intact", () => {
  ...
  assertTrue(typeof flight.totalElapsedTime === "number", "flight has totalElapsedTime");
...
test("processDepositAction handles missing destination station gracefully (cargo silently discarded, reservation released)", () => {
  // Simulates a destination demolished mid-trip: ship's cargo-deposit action
  // resolves to a station whose slot is gone. processDepositAction's
  // missing-slot guard discards cargo and releases the reservation without
  // crashing — pin that with a 1-leg trip whose destination we mutate.
  const simulation = freshSettledSimulation();
  const candidate = findShipWithNaturalTrip(simulation);
  assertTrue(candidate !== null, "natural trip");
  if (!candidate) { simulation.tradeManager.dispose(); return; }
  ...
  // Demolish the destination station mid-trip — remove its slots so the
  // cargo-deposit's getInventorySlot returns undefined.
  const destination = legs[0].toStation;
  destination.inventory = [];
  destination.inventoryByWareId.clear();

  // Tick to completion. Pin: no crash, ship returns home, cargo cleared.
  for (let stepIndex = 0; stepIndex < 4000; stepIndex++) {
    simulation.tradeManager.tick(1);
  }

  // Ship cargo cleared (silent discard fallback).
  let totalCargo = 0;
  for (const amount of ship.cargoAmountByWareId.values()) totalCargo += amount;
  assertEqual(totalCargo, 0, "cargo silently discarded when destination slots gone");
...
test("1-leg buy trip: outbound fly leaves from orbit (no home cargo to load → no surface dock at home)", () => {
  ...
  if (firstInterStationFly && firstInterStationFly.type === "fly") {
    assertEqual(firstInterStationFly.origin.surfaceOrOrbit, "orbit", "outbound fly leaves from orbit when no home cargo to load");

...
test("createStationHistory: empty state returns empty stations", () => {
  ...
  const frame = history.getStateAt(0);
  assertEqual(frame.simSeconds, 0, "simSeconds");
  assertEqual(frame.stations.length, 0, "stations.length");
});

test("createStationHistory: created station appears at and after its event time", () => {
  const history = createStationHistory();
  history.recordCreated(100, stationFixture());
  assertEqual(history.getStateAt(99).stations.length, 0, "before event");
  assertEqual(history.getStateAt(100).stations.length, 1, "at event");
  const station = history.getStateAt(100).stations[0];
  ...
  assertEqual(history.getStateAt(200).stations.length, 1, "after event");
});

test("createStationHistory: removed event drops only the named station", () => {
  const history = createStationHistory();
  history.recordCreated(100, stationFixture({ id: "hub-tech-1" }));
  history.recordCreated(100, stationFixture({ id: "hub-tech-2" }));
  history.recordRemoved(200, "hub-tech-1");
  assertEqual(history.getStateAt(150).stations.length, 2, "before removal");
  ...
  const survivors = history.getStateAt(200).stations;
...
test("createStationHistory: state-changed updates state without losing other fields", () => {
  const history = createStationHistory();
  history.recordCreated(100, stationFixture({ state: "construction" }));
  history.recordStateChanged(200, "hub-tech-1", "operational");
  const earlier = assertNotUndefined(history.getStateAt(150).stations[0], "earlier");
  const later = assertNotUndefined(history.getStateAt(250).stations[0], "later");
...
test("createStationHistory: counts per nation excluding WAY", () => {
  ...
  history.recordCreated(0, stationFixture({ id: "b", nationId: "bio" }));
  history.recordCreated(0, stationFixture({ id: "c", nationId: "way" }));
...
test("createStationHistory: round-trips through toSnapshot / fromSnapshot", () => {
  const history = createStationHistory();
  history.recordCreated(50, stationFixture());
  ...
  const original = history.getStateAt(200).stations[0];
  const after = restored.getStateAt(200).stations[0];
